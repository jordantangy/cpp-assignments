...
void WarGame::Board::move(uint player_number, std::pair<int,int> source, WarGame::Board::MoveDIR direction){
    ...
    /*
        If the soldier we are trying to attack is upward we check if the upper place in the matrix is null, then 
        if it is null, we can move the soldier there and mettre a zero lendroit dou il est venu.
        then we enable the activity.
    */
    ...
    bool WarGame::Board::has_soldiers(uint player_number) const{
        for (size_t i = 0; i < board.size(); i++) {
            for (size_t j = 0; j < board[i].size(); j++) {
                if (board[i][j] != nullptr && player_number == board[i][j]->get_player()){
                    return true;
                } 
            }
        }
